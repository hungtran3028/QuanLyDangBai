...
- App chính thức qua Tailscale: `https://hungtran.tail07d810.ts.net`
- Facebook OAuth callback bắt buộc: `https://hungtran.tail07d810.ts.net/api/facebook/callback`
...
3. Sửa `.env` theo cấu hình thật. Với dự án này, luôn giữ:
...
- Ép `APP_BASE_URL` về `https://hungtran.tail07d810.ts.net`.
- Ép `PORT=3000`.
...
```text
https://hungtran.tail07d810.ts.net
```
...
Chỉ mở `http://localhost:3000` để kiểm tra nội bộ. Khi dùng Facebook OAuth, phải dùng URL Tailscale ở trên.
...
2. Đảm bảo app đang chạy qua `https://hungtran.tail07d810.ts.net`.
3. Trong Meta App, cấu hình redirect URI:

```text
https://hungtran.tail07d810.ts.net/api/facebook/callback
```
...
- `.env` có `APP_BASE_URL=https://hungtran.tail07d810.ts.net`.
- Meta App có redirect URI `https://hungtran.tail07d810.ts.net/api/facebook/callback`.
